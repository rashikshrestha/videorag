...
def main():
    ...
    output_path = Path("data/queries_processed.yaml")
    ...
    for episode, start, end, concept in flat_rows:
        canon = canonical_map[concept]
        grouped[canon].append({"episode": int(episode), "start": start, "end": end})

    records = [{"query": q, "timestamps": ts} for q, ts in grouped.items()]
